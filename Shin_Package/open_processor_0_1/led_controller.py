...
import Shin_Package.open_processor_0_1.led_controll as led
import Shin_Package.open_processor_0_1.led_selecter as selecter


# 잠금 초기화 하는 함수[조명 업,다운, 센서]
from Shin_Package.open_processor_0_1 import printer
...
def control_led(led_num, control_step):
    locking_init_led()
    led.set_LED(led_num, datas.led_control[control_step][0], datas.led_control[control_step][1], datas.led_control[control_step][2],
...
def illum_before_needs(II_illum, avg_illum):
    ...
    led_up(min_sensor, datas.sensor_influence_sum_part[min_sensor], datas.sensor_influence_value_sum_part[min_sensor], control_step)
...
def illum_match_needs(II_illum, avg_illum):
    ...
    for i in range(9):
        if (datas.sensor_up_lock[i] != 1 & (II_illum[i] - datas.object_illum < 0)) | (
                datas.sensor_down_lock[i] != 1 & (II_illum[i] - datas.object_illum > 0)):
...
def illum_after_needs(II_illum, avg_illum):
    ...
    if max_sensor == -1:
        ...
        led_down(max_sensor, datas.sensor_influence_sum_part_reverse[max_sensor], datas.sensor_influence_value_sum_part_reverse[max_sensor], control_step)
...
def dir_finder(is_Low, select, II_illum):
    ...
    square_ii = II_illum.reshape(3, 3)
...
def led_down(max_sensor, influence_rank, influence_value, control_step=1):
    ...
    control_led(control_led_idx, datas.led_state[control_led_idx])
        ...
        for i in range(1, len(datas.sensor_influence_sum_part[j])):
            while datas.led_state[datas.sensor_influence_sum_part[j][i]] > 1:
                ...
                control_led(datas.sensor_influence_sum_part[j][target_led_rank],
                            datas.led_state[datas.sensor_influence_sum_part[j][target_led_rank]])
                control_led(datas.sensor_influence_sum_part[j][i], datas.led_state[datas.sensor_influence_sum_part[j][i]])
            ...
            if (j < len(datas.sensor_influence_sum_part[i]) - 1):
                while canControl:
                    for k in range(j + 1, len(datas.sensor_influence_sum_part[i])):
                        ...
                        print(
                            "Control Index  %s %s" % (datas.sensor_influence_sum_part[i][j], datas.sensor_influence_sum_part[i][k]))
                        print("influence cmp %s <= %s <= %s" % (
                            ...
                            str(datas.sensor_influence_value_sum_part[i][j] + datas.similar_led_range)))
                        print("State cmp %s > %s" % (
                            datas.led_state[datas.sensor_influence_sum_part[i][j]], datas.led_state[datas.sensor_influence_sum_part[i][k]]))
                        if ((datas.sensor_influence_value_sum_part[i][j] - datas.similar_led_range) <=
                            datas.sensor_influence_value_sum_part[i][
                                k]) & (
                                datas.sensor_influence_value_sum_part[i][k] <= (datas.sensor_influence_value_sum_part[i][
                            ...
                            print("GIVE 1 STEP LED %s[%s] -> LED %s[%s]" % (
                                datas.sensor_influence_sum_part[i][j], datas.led_state[datas.sensor_influence_sum_part[i][j]],
                                datas.sensor_influence_sum_part[i][k], datas.led_state[datas.sensor_influence_sum_part[i][k]]))
