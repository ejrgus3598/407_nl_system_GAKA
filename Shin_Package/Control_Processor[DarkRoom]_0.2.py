...
import Shin_Package.shin_Intsain_LED as led
...
def illum_before_needs(II_illum):
    ...
    led_up(min_sensor, sensor_influence_sum_part, sensor_influence_value_sum_part, first_matched)
...
def illum_match_needs(II_illum, avg_illum):
    ...
    if diff_value >= 0:
        # 조명 선정 및 제어
        control_led = led_down(diff_max_sensor, sensor_influence_all, sensor_influence_value_all, first_matched)
    ...
    # 평균조도와 지점 조도의 차이가 음수인 경우 조도가 부족하므로 상향점등 후 해당 조명 하향점등 잠금
        ...
        # 조명 선정 및 제어
        control_led = led_up(diff_max_sensor, sensor_influence_all, sensor_influence_value_all, first_matched)
...
def illum_after_needs(II_illum):
    ...
    led_down(max_sensor, sensor_influence_sum_part_reverse, sensor_influence_value_sum_part_reverse, first_matched)
...
def led_down(max_sensor, influence_rank, influence_value):
    ...
    find_similar_influence(max_sensor, control_led_rank, influence_rank, influence_value, False)
...
def led_up(min_sensor, influence_rank, influence_value):
    ...
    #         control_led_rank = j
    #         print("CHANGE(SIMILAR) LED NUM : %s" % control_led_idx)
    find_similar_influence(min_sensor, control_led_rank, influence_rank, influence_value, True)
...
def select_control_led(sensor, now_rank, influence_rank, isLow):
    ...
    if (led_lock_up[control_led_idx] == 1) | (led_state[control_led_idx] == len(led_control) - 1):
        print(
            "LOCKING DETECTED[%s] or LED IS MAX STATE[%s]" % (led_lock_up[control_led_idx], led_state[control_led_idx]))
        for i in range(1, len(influence_rank[sensor])):
            if (led_lock_up[influence_rank[sensor][i]] == 0):
                if isLow & (led_state[influence_rank[sensor][i]] != len(led_control) - 1):
                    control_led_idx = influence_rank[sensor][i]
                    control_led_rank = i
                    print("SELECT LED NUM : %s" % control_led_idx)
                    break
                elif not False & (led_state[influence_rank[sensor][i]] != 0):
                    control_led_idx = influence_rank[sensor][i]
                    control_led_rank = i
                    print("SELECT LED NUM : %s" % control_led_idx)
                    break
    ...
    for j in range(control_led_rank + 1, len(influence_rank[sensor])):
        if (influence_value[sensor][control_led_rank] - similar_led_range <=
            influence_value[sensor][j]) & (
                ...
                led_lock_up[influence_rank[sensor][j]] == 0):
...
# [0.2]조도 만족시키기 전 제어상태를 확인하고 총영향도에 기반한 제어상태로 돌리기
# 한단계 씩 유사 영향도를 검사해서 찾아야 할듯 함.
def led_setting_based_on_influence_sum(isLow):
    print("First Step Setting")
    print_control_lux()

    if isLow:
        print('''We don't have enough illumination.
                Applied in the order of total impact.
                I'll control the lighting.''')
        for j in range(len(sensor_influence_sum_part)):
            for i in range(1, len(sensor_influence_sum_part[j])):
                print()
    else:
        print('''There's enough light.
                Applied in reverse order of total impact.
                I'll control the lighting.''')
        for j in range(len(sensor_influence_sum_part)):
            for i in range(1, len(sensor_influence_sum_part_reverse[j])):
                print()
