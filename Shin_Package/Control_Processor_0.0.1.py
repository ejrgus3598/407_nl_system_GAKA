...
def process():
    ...
    while True:
        ...
        print(
            "Avg_CCT\t\t%s\t\tAvg_Ill\t\t%s\t\tSum_Curr\t\t%s\t\tUniformity\t\t%s" % (str(avg_cct)[:7], str(avg_illum)[:8], str(sum_curr), str(uniformity)))
...
def save_data(df_record, acs_cct, II_illum, IC_curr, avg_cct, avg_illum, sum_curr, uniformity, get_times, save_name, isPrint):
...
def led_up(min_sensor, influence_rank, influence_value):
    ...
    if (led_lock_up[control_led_idx] == 1) | (led_state[control_led_idx] == len(led_control) - 1):
        ...
        for i in range(len(influence_rank[min_sensor])):
